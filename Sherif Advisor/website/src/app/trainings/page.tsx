...
import { Calendar, MapPin, Clock, ArrowRight, Plus, ChevronLeft, ChevronRight, GraduationCap } from 'lucide-react';
...
const faqs = [
  { question: 'When is the best time of year to attend advisory trainings in Egypt?', answer: 'The peak season for corporate tax and legal training sessions in Egypt is typically between October and April, aligning with the fiscal year-end and new regulatory implementations.' },
  { question: 'Are the training webinars recorded for later viewing?', answer: 'Yes, all virtual training webinars are recorded and provided exclusively to registered attendees within 48 hours of the live session.' },
  ...
  { question: 'Where can I register for official trainings?', answer: 'Official registration is available through the registration links provided on each training listing on this page.' },
  { question: 'Do I need to book seats for trainings in advance?', answer: 'Yes, early booking is highly recommended as our specialized workshops and seminars have limited capacity and often sell out.' },
  { question: 'What happens if a training is cancelled or rescheduled?', answer: 'If a training session is cancelled or rescheduled, registered attendees will be notified via email with options for a full refund or transfer to the new date.' },
];
...
export default function TrainingsPage() {
  ...
  const [featuredTrainings, setFeaturedTrainings] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (featuredTrainings.length === 0) return;
    const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % featuredTrainings.length);
    }, 6000);
  ...
  const filteredTrainings = activeCategory === 'all' 
    ? trainings 
    : trainings.filter((event: any) => event.metadata?.category === activeCategory);

  if (isLoading) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-[#009086] text-xl font-bold animate-pulse">Loading trainings...</div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-white text-brand-navy font-sans">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <div className="p-8 lg:p-12 flex flex-col justify-center min-h-[400px] lg:min-h-[500px]">
                <div className="flex items-center gap-3 mb-4">
                  <span className="bg-brand-navy text-white text-[10px] font-bold tracking-wider uppercase px-3 py-1.5 rounded-full flex items-center gap-1.5">
                    <GraduationCap className="w-3 h-3" /> Featured Training
                  </span>
                  <span className="text-gray-500 text-xs font-bold tracking-wider uppercase">
                    {featuredTrainings[currentSlide]?.metadata?.startDate ? new Date(featuredTrainings[currentSlide].metadata.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'TBA'}
                  </span>
                </div>
                <h2 className="font-sans text-2xl lg:text-3xl font-bold text-brand-navy mb-4 leading-snug">
                  {featuredTrainings[currentSlide]?.title}
                </h2>
                <p className="text-gray-600 text-sm leading-relaxed mb-6 max-w-lg">
                  {featuredTrainings[currentSlide]?.body?.substring(0, 150)}...
                </p>
                <div className="flex flex-wrap items-center gap-4 mb-8 text-xs text-gray-600 font-bold">
                  <div className="flex items-center gap-2 bg-gray-100 px-3 py-1.5 rounded-full">
                    ...
                    <span>{featuredTrainings[currentSlide]?.metadata?.startTime || 'TBA'} - {featuredTrainings[currentSlide]?.metadata?.endTime || 'TBA'}</span>
                  </div>
                  <div className="flex items-center gap-2 bg-gray-100 px-3 py-1.5 rounded-full">
                    <MapPin className="w-4 h-4 text-[#009086]" />
                    <span>{featuredTrainings[currentSlide]?.metadata?.location || 'TBA'}</span>
                  </div>
                </div>
                <Link href={`/trainings/${featuredTrainings[currentSlide]?.id}`} className="inline-flex items-center gap-2 bg-brand-navy hover:bg-[#009086] text-white px-6 py-3 rounded-full text-xs font-bold tracking-wider uppercase transition-all duration-300 ease-in-out hover:shadow-lg w-fit">
              ...
              <div className="relative h-64 lg:h-auto min-h-[300px] lg:min-h-[500px] bg-gray-100 overflow-hidden">
                ...
                <button onClick={() => setCurrentSlide((prev) => (prev + 1) % featuredTrainings.length)} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/90 backdrop-blur-sm border border-gray-200 hover:bg-brand-navy hover:text-white hover:border-brand-navy rounded-full flex items-center justify-center shadow-lg z-20 transition-all duration-300 ease-in-out">
        ...
        <section id="upcoming" className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
              <h2 className="text-4xl font-bold text-brand-navy leading-10">Upcoming trainings</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTrainings.map((event: any, index) => {
      ...
      {/* ── Top Trainings (Brand Navy Background for Contrast) ── */}
      <FadeIn>
        <section className="py-20 bg-brand-navy relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <h2 className="text-4xl font-bold text-white leading-10 mb-10">Top rated trainings</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {trainings.slice(0, 3).map((event: any, index) => {
                const meta = event.metadata || {};
                return (
                  <FadeIn key={event.id} delay={index * 150}>
                    <Link href={`/trainings/${event.id}`} className="relative group block">
