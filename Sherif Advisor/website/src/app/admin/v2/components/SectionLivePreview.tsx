...
function resolvePublicUrl(page?: string, sectionKey?: string): string {
  ...
  switch (page) {
    case 'v2':
      // V2 landing page sections are anchored on the /v2 single-page layout.
      if (sectionKey === 'hero') return '/v2';
      if (sectionKey === 'journey') return '/v2#journey';
      if (sectionKey === 'services') return '/v2#services';
      if (sectionKey === 'markets') return '/v2#markets';
      if (sectionKey === 'digital') return '/v2#digital';
      if (sectionKey === 'packages') return '/v2#packages';
      if (sectionKey === 'contact') return '/v2#contact';
      return '/v2';
    case 'homepage':
...
export default function SectionLivePreview({
  ...
  // The target anchor id (e.g. "services" from "/v2#services"). Empty for
  // top-of-page sections like the hero.
  const anchorId = url.includes('#') ? url.split('#')[1] : '';

  /**
   * After the iframe loads, scroll it to the section being edited. The public
   * pages are client-rendered single-page layouts, so the browser's native
   * hash scroll fires before the section mounts. We retry a few times to catch
   * the element once React has painted it. Same-origin (localhost) access is
   * allowed, so this is safe here.
   */
