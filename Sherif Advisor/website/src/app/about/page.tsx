...
interface CmsSection {
  title: string;
  body: string;
  metadata: { fields?: Record<string, { ar: string; en: string }> };
}
...
const stats = [
  ...
  { valueEn: '500+', valueAr: '+500', labelEn: 'Clients Served', labelAr: 'عميل' },
...
const perks = [
  ...
  { icon: Users,      titleEn: 'Collaborative Culture', titleAr: 'ثقافة تعاونية',     descEn: 'Work alongside top experts and share ideas across departments.',               descAr: 'العمل جنباً إلى جنب مع كبار الخبراء وتبادل الأفكار.' },
...
/* ── Helper: read a CMS field ──────────────────────────────────── */
function field(section: CmsSection | null, key: string, lang: Language, fallbackAr: string, fallbackEn: string) {
...
export default function AboutPage() {
  ...
  useEffect(() => { setLang(getClientLanguage()); setLangReady(true); }, []);
    ...
    <main className="min-h-screen bg-white" dir={dir}>
      ...
      <section className="relative h-[70vh] min-h-[560px] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src="/images/bg-3.jpeg" alt="About us" fill className="object-cover" priority />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-24">
          ...
          <h1 className="font-serif text-white leading-[1.05] max-w-2xl mb-6"
            ...
            <br />
            <span className="italic text-brand-gold">{t(lang, 'نحو القرار الصائب.', 'Guiding Every Decision.')}</span>
          </h1>
          <p className="text-gray-300 text-base max-w-xl leading-relaxed mb-8"
            style={{ animation: 'aboutFadeUp 0.8s ease 0.6s both' }}
            dangerouslySetInnerHTML={{
              __html: heroSection?.body || t(lang,
                'شريف يسري للاستشارات — شركة استشارية متخصصة تقدم خدمات الضرائب والتمويل وإدارة الأعمال.',
                'Sherif Yousry Advisory — a specialised firm delivering tax, finance, and business management services across Egypt and the MENA region.'
              )
            }}
          />
          {/* Stats */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <FadeIn delay={200}>
              <p className="font-mono text-xs tracking-[0.3em] text-brand-gold uppercase mb-4">
                {field(storySection, 'eyebrow', lang, 'قصتنا', 'OUR STORY')}
              </p>
              ...
              {storySection?.body ? (
                ...
                <div className="space-y-4 text-gray-600 leading-relaxed text-[15px]">
                  ...
                  <p>{t(lang, 'ما يميّزنا هو التزامنا بالجودة والنزاهة، ورؤيتنا الشاملة التي تجمع بين الخبرة المحلية والمنظور الدولي.', 'What sets us apart is our commitment to quality and integrity, and our holistic view combining local expertise with an international perspective.')}</p>
                </div>
              )}
            </FadeIn>
          </div>
      ...
      {/* ══ MISSION & VISION ══════════════════════════════════════ */}
      <section id="mission-vision" className="py-24"
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <FadeIn className="text-center mb-16">
            <p className="font-mono text-xs tracking-[0.3em] text-brand-gold uppercase mb-4">
              {field(missionSection, 'eyebrow', lang, 'رسالتنا ورؤيتنا', 'MISSION & VISION')}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <FadeIn delay={100}>
              <div className="relative rounded-sm border border-white/10 bg-white/5 backdrop-blur-sm p-10 h-full group hover:border-brand-gold/40 transition-colors duration-300">
                ...
                <h3 className="font-serif text-2xl text-white mb-4">
                  {field(missionSection, 'missionTitle', lang, 'الرسالة', 'Our Mission')}
                </h3>
                <p className="text-gray-300 leading-relaxed">
                  {missionSection?.body || t(lang, 'تمكين الشركات والمستثمرين والأفراد من اتخاذ قرارات مالية وتجارية مدروسة.', 'To empower businesses, investors, and individuals to make informed financial and commercial decisions through integrated advisory services.')}
                </p>
              </div>
            </FadeIn>
            ...
            <FadeIn delay={200}>
              <div className="relative rounded-sm border border-white/10 bg-white/5 backdrop-blur-sm p-10 h-full group hover:border-brand-gold/40 transition-colors duration-300">
                ...
                <h3 className="font-serif text-2xl text-white mb-4">
                  {field(missionSection, 'visionTitle', lang, 'الرؤية', 'Our Vision')}
                </h3>
                <p className="text-gray-300 leading-relaxed">
                  {field(missionSection, 'visionBody', lang,
      ...
      <section id="careers" className="py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <FadeIn className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-16">
            <div>
              <p className="font-mono text-xs tracking-[0.3em] text-brand-gold uppercase mb-4">
                {field(careersSection, 'eyebrow', lang, 'الوظائف', 'CAREERS')}
              </p>
              <h2 className="font-serif text-brand-navy leading-tight" style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)' }}>
                {careersSection?.title || (
                  <>{t(lang, 'انضم إلى فريقنا', 'Join Our Team')}<br />
                  <span className="italic text-brand-gold">{t(lang, 'واصنع مستقبلك معنا.', 'Build Your Future With Us.')}</span></>
                )}
              </h2>
            </div>
            <div className="flex items-end">
              <Link href="/careers"
                ...
                <span className="relative z-10 text-brand-navy transition-colors duration-300 group-hover:text-white">
                  {field(careersSection, 'ctaLabel', lang, 'استعرض الوظائف', 'View All Openings')}
                </span>
              </Link>
            </div>
          </FadeIn>
              ...
              <div className="relative z-10 py-16 px-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div>
                  <h3 className="font-serif text-2xl md:text-3xl text-white mb-2">
                    {field(careersSection, 'bannerHeading', lang, 'هل أنت مستعد للخطوة التالية؟', 'Ready to take the next step?')}
                  </h3>
                  <p className="text-gray-300 text-sm leading-relaxed max-w-lg">
                    {field(careersSection, 'bannerBody', lang,
                ...
                <Link href="/careers#apply-now"
                  ...
                  <span className="relative z-10 text-white transition-colors duration-300 group-hover:text-[#030a12]">
                    {field(careersSection, 'applyLabel', lang, 'قدّم الآن', 'Apply Now')}
                  </span>
                </Link>
              </div>
            </div>
